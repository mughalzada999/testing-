...
#LOGO-BANNER
def banner():
	cetak(nel('    Simple Crack Facebook [blue]Pembuat Scrip By : [green]Resyah [red]Coder'))
#BAGIAN-MASUK
def login():
	try:
...
def login_lagi334():
	try:
		...
		print(f'  {x}[{h}•{x}]{h} LOGIN BERHASIL TUNGGU SEBENTAR{x} ');time.sleep(1)
		exit()
	except Exception as e:
...
def result():
	...
	if kz in ['2']:
		try:vin = os.listdir('CP')
			...
			print('[{b}•{x}]{h} File Tidak Di Temukan ')
			time.sleep(3)
			back()
		if len(vin)==0:
			print('[{b}•{x}]{h} Anda Tidak Memiliki Hasil CP ')
			time.sleep(2)
			back()
		else:
			cih = 0
...
def dump_massal():
	...
	for userr in uid:
		try:
			...
			exit()
	try:
		print(x+'['+h+'•'+x+'] Total Id yang terkumpul : \x1b[1;92m'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r ─ {h}{P}{b}{loop}{P}/{P}{len(id)}{P} {H}OK:{P}{ok}{P} {k}CP:{P}{cp}{x}  {P}{'{:.0%}'.format(loop/float(len(id)))}{P}  "),
	sys.stdout.flush()
